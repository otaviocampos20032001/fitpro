...
export default async function AlunosPage() {
  ...
  function getLastSession(studentId: string) {
    return sessions?.find((s) => s.student_id === studentId);
  }
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 16 }}>
        <div>
          ...
          <p style={{ color: "var(--text-secondary)", marginTop: 4, fontSize: 14 }}>
            {students?.length || 0} aluno{(students?.length || 0) !== 1 ? "s" : ""} cadastrado{(students?.length || 0) !== 1 ? "s" : ""}
          </p>
        </div>
      ...
      {!students || students.length === 0 ? (
        <div style={{ textAlign: "center", padding: "80px 20px" }}>
          ...
          <p style={{ color: "var(--text-secondary)", fontSize: 14, marginBottom: 24 }}>
            Adicione seu primeiro aluno para começar a gerenciar treinos.
          </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
          {students.map((student) => {
            const lastSession = getLastSession(student.id);
            const daysAgo = getDaysAgo(lastSession?.created_at);
            const isInactive = daysAgo === null || daysAgo > 5;
            const totalSessions = sessions?.filter((s) => s.student_id === student.id && s.status === "completed").length || 0;
                ...
                <div className="glass-card glass-card-hover" style={{ padding: 20 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div style={{ background: "var(--surface-2)", borderRadius: 10, padding: "10px 12px" }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)" }}>{totalSessions}</div>
                    ...
                    <div style={{ background: "var(--surface-2)", borderRadius: 10, padding: "10px 12px" }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: isInactive ? "var(--orange)" : "var(--green)" }}>
                        {daysAgo === null ? "Nunca" : daysAgo === 0 ? "Hoje" : `${daysAgo}d atrás`}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--text-muted)" }}>Último treino</div>
                    </div>
                  </div>

                  {student.goal && (
                    <div style={{ marginTop: 12, fontSize: 12, color: "var(--text-secondary)", padding: "8px 10px", background: "var(--surface-2)", borderRadius: 8, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      🎯 {student.goal}
                    </div>
                  )}
