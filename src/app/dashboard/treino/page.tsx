...
export default function TreinoPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: plan } = await supabase.from("workout_plans").select("*, workout_days(*, workout_day_exercises(*, exercises(*)))").eq("student_id", uid).eq("active", true).order("created_at", { ascending: false }).limit(1).maybeSingle();
